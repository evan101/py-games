...
MAXHEALTH = 4        # how much health the player starts with
...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, L_BANANA_IMG, R_BANANA_IMG, L_SQUIR_IMG, R_SQUIR_IMG, GRASSIMAGES

    pygame.init()
    FPSCLOCK = pygame.time.Clock()
    pygame.display.set_icon(pygame.image.load('gameicon.png'))
    DISPLAYSURF = pygame.display.set_mode((WINWIDTH, WINHEIGHT))
    pygame.display.set_caption('Minion eat banana')
    BASICFONT = pygame.font.Font('freesansbold.ttf', 32)

    # load the image files
    L_SQUIR_IMG = pygame.image.load('minion-cake.png')
    R_SQUIR_IMG = pygame.transform.flip(L_SQUIR_IMG, True, False)

    L_BANANA_IMG = pygame.image.load('banana.png')
    R_BANANA_IMG = pygame.transform.flip(L_BANANA_IMG, True, False)
    GRASSIMAGES = []
    for i in range(1, 5):
...
def runGame():
    ...
    winSurf = BASICFONT.render('You have achieved OMEGA MINION!', True, WHITE)
        ...
        for sObj in squirrelObjs:
            ...
            if random.randint(0, 99) < DIRCHANGEFREQ:
                ...
                if sObj['movex'] > 0: # faces right
                    sObj['surface'] = pygame.transform.scale(R_BANANA_IMG, (sObj['width'], sObj['height']))
                else: # faces left
                    sObj['surface'] = pygame.transform.scale(L_BANANA_IMG, (sObj['width'], sObj['height']))
